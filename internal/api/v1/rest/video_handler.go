...
func (h *handler) CreateVideo(c echo.Context) error {
	...
	createdVideo, err := h.videoUsecase.VideoCreate(videoFileData)
	if err != nil {
	...
	createdVideoDescription, err := h.videoUsecase.VideoDescription(videoDescriptionData)
	if err != nil {
...
func (h *handler) GetVideos(c echo.Context) error {
	...
	videoData, err := h.videoUsecase.GetVideoList(videoKategori, videoKelas)
	if err != nil {
...
func (h *handler) GetVideoByID(c echo.Context) error {
	...
	video, err := h.videoUsecase.GetVideoByID(videoID)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]interface{}{
			"error": err.Error(),
		})
...
func (h *handler) DownloadVideo(c echo.Context) error {
	videoID := c.Param("id")
	fileName, err := h.videoUsecase.GetFileNameByID(videoID)
	if err != nil {
...
func (h *handler) EditVideo(c echo.Context) error {
	...
	existingVideo, err := h.videoUsecase.GetVideoByID(videoID)
	if err != nil {
		return c.JSON(http.StatusNotFound, map[string]string{"error": "Video not found"})
	}
	...
	updatedVideo, err := h.videoUsecase.UpdateVideo(existingVideo)
	if err != nil {
